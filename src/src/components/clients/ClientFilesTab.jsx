...
export default function ClientFilesTab({ client, canEdit = false }) {
    ...
    try {
      ...
      for (const file of nextFiles) {
        if (file.size > MAX_FILE_BYTES) {
          showToast(`${file.name} ultrapassa 10 MB.`, { variant: 'error' });
          continue;
        }
  ...
  return (
    <div className={styles.panel} onPaste={handlePaste}>
      <div className={styles.header}>
        <div>
          <h3>Arquivos do cliente</h3>
          <span>Anexe documentos, prints e materiais operacionais vinculados ao cliente.</span>
        </div>
        {canEdit ? (
      ...
      {canEdit ? (
        ...
        >
          ...
          <span>Limite por arquivo: 10 MB.</span>
        </div>
      ) : null}
