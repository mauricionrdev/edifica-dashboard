...
function buildKpiDelta(currentValue, previousValue, formatter, options = {}) {
  ...
  const diff = cur - prev;
  if (Math.abs(diff) < 0.0001) return { delta: '0', deltaTone: 'flat' };
...
function MetricCard({
  ...
  return (
    <article
      className={`${styles.metricCard} ${styles[`metricCard_${tone}`] || styles.metricCard_neutral} ${dragging ? styles.metricCardDragging : ''}`.trim()}
      draggable={draggable}
    ...
    >
      ...
      {delta || helper ? (
        <p className={styles.metricFooter}>
          {delta ? (
            <span className={`${styles.metricDelta} ${styles[`metricDelta_${deltaTone || 'neutral'}`] || ''}`.trim()}>
              {delta}
            </span>
          ) : null}
      ...
      {typeof progress === 'number' ? (
        <div className={styles.metricProgress}>
          <span
            className={`${styles.metricProgressBar} ${styles[`metricProgressBar_${progressTone || tone}`] || ''}`.trim()}
    ...
    <div className={styles.columnsPanel}>
      ...
      <div className={styles.columnsCanvas}>
        <svg className={styles.columnsSvg} viewBox={`0 0 ${VB_W} ${VB_H}`} preserveAspectRatio="none" aria-label="Entradas por mês">
          <defs>
            <linearGradient id="designLabEntriesAreaFade" x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stopColor="rgba(255,255,255,0.055)" />
              <stop offset="100%" stopColor="rgba(255,255,255,0)" />
            </linearGradient>
            <linearGradient id="designLabEntriesLineFade" x1="0" x2="1" y1="0" y2="0">
          ...
          {ticks.map((tick) => {
            ...
            return (
              <g key={`grid-${tick}`}>
                ...
                <text x={padding.left - 42} y={y + 4} textAnchor="end" className={styles.columnsAxisText}>{tick}</text>
              </g>
            );
          })}

          <path d={areaPath} fill="url(#designLabEntriesAreaFade)" />
          <path d={linePath} fill="none" stroke="url(#designLabEntriesLineFade)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />

          <line x1={currentPoint.px} x2={currentPoint.px} y1={padding.top} y2={padding.top + plotH} stroke="rgba(245,184,0,0.12)" strokeWidth="1" vectorEffect="non-scaling-stroke" />

          {points.map((point) => {
            const isCurrent = Boolean(point.isNow);
            return (
              <g key={`${point.y}-${point.m}`}>
                <circle cx={point.px} cy={point.py} r={isCurrent ? '12' : '8'} fill={isCurrent ? 'rgba(245,184,0,0.08)' : 'rgba(255,255,255,0.035)'} />
                <circle cx={point.px} cy={point.py} r={isCurrent ? '6' : '4'} fill={isCurrent ? 'rgba(245,184,0,0.92)' : 'rgba(232,234,238,0.82)'} />
                <text x={point.px} y={point.py - 16} textAnchor="middle" className={`${styles.columnsValue} ${isCurrent ? styles.columnsValueCurrent : ''}`.trim()}>{fmtInt(point.cnt)}</text>
                <text x={point.px} y={VB_H - 28} textAnchor="middle" className={`${styles.columnsMonth} ${isCurrent ? styles.columnsMonthCurrent : ''}`.trim()}>{MONTHS_FULL[point.m].slice(0, 3).toUpperCase()}</text>
                <text x={point.px} y={VB_H - 12} textAnchor="middle" className={styles.columnsYear}>{String(point.y)}</text>
              </g>
            );
          })}
        </svg>
...
function ComparisonPanel({ current, previous, currentLabel }) {
  const buildRow = (label, currentVal, previousVal, formatter, options = {}) => {
    ...
    let tone;
    if (isFlat) tone = 'flat';
    else if (options.invert) tone = diff < 0 ? 'good' : 'risk';
    else tone = diff > 0 ? 'good' : 'risk';
          ...
          {rows.map((row) => (
            <div key={row.label} className={styles.compareItem}>
              <div className={styles.compareLeft}>
                ...
                <span className={`${styles.compareDelta} ${styles[`compareDelta_${row.tone}`] || ''}`.trim()}>{row.delta}</span>
              </div>
  ...
  return (
    <div className={styles.page}>
      <section className={styles.metricsGrid}>
        {Array.from({ length: 8 }).map((_, index) => <div key={index} className={`${styles.metricCard} ${styles.skeletonBlock}`} />)}
      </section>
...
export default function DesignLabDashboardPage() {
  ...
  const clientOptions = useMemo(() => {
    ...
    const filtered = squadFilter ? list.filter((client) => (client.squadId || client.squad_id) === squadFilter) : list;
    return [...filtered].filter((client) => client?.name).sort((a, b) => String(a.name).localeCompare(String(b.name), 'pt-BR'));
  }, [clients, squadFilter]);
  ...
  const executiveMetrics = useMemo(() => computeCentralMetrics(visibleClients, period.y, period.m), [visibleClients, period]);
  const prevPeriod = useMemo(() => previousPeriod(period.y, period.m), [period]);
  const previousMetrics = useMemo(() => computeCentralMetrics(visibleClients, prevPeriod.y, prevPeriod.m), [prevPeriod, visibleClients]);
  ...
  const entryColumns = useMemo(() => buildBarChartData(visibleClients, period.y, period.m, 6), [period, visibleClients]);
  const recentActivities = useMemo(() => buildClientActivities(visibleClients), [visibleClients]);
  ...
  const metricDefinitions = useMemo(() => {
    ...
    return [
      { id: 'base', label: 'Base de dados', value: fmtInt(totalClients), helper: 'vs. mês passado', delta: baseDelta.delta, deltaTone: baseDelta.deltaTone, icon: <UsersIcon size={14} />, tone: 'neutral' },
      { id: 'ativos', label: 'Clientes ativos', value: fmtInt(activeClients), helper: 'vs. mês passado', delta: ativosDelta.delta, deltaTone: ativosDelta.deltaTone, icon: <BriefcaseIcon size={14} />, tone: 'neutral' },
      { id: 'novosAtual', label: 'Clientes novos no mês', value: fmtInt(currentMonthNewClients), helper: 'vs. mês passado', delta: novosDelta.delta, deltaTone: novosDelta.deltaTone, icon: <TrendingUpIcon size={14} />, tone: currentMonthNewClients > 0 ? 'good' : 'neutral' },
      { id: 'mrr', label: 'MRR atual', value: fmtMoney(mrr), helper: 'vs. mês passado', delta: mrrDelta.delta, deltaTone: mrrDelta.deltaTone, icon: <CoinsIcon size={14} />, tone: 'neutral' },
      { id: 'receitaNova', label: 'Receita nova gerada', value: fmtMoney(revenueNew), helper: 'vs. mês passado', delta: receitaNovaDelta.delta, deltaTone: receitaNovaDelta.deltaTone, icon: <TrendingUpIcon size={14} />, tone: revenueNew > 0 ? 'good' : 'neutral' },
      { id: 'ticket', label: 'Ticket médio', value: fmtMoney(ticketMedio), helper: 'vs. mês passado', delta: ticketDelta.delta, deltaTone: ticketDelta.deltaTone, icon: <TargetIcon size={14} />, tone: 'neutral' },
      { id: 'perdida', label: 'Receita perdida no mês', value: fmtMoney(revenueLost), helper: revenueLost > 0 ? `${fmtInt(churnedPeriod)} churn(s) em ${MONTHS_FULL[period.m]}` : 'sem perdas', delta: revenueLost > 0 ? perdidaDelta.delta : '', deltaTone: perdidaDelta.deltaTone, icon: <ChartColumnIcon size={14} />, tone: revenueLost > 0 ? 'risk' : 'neutral' },
      { id: 'churn', label: 'Taxa de churn', value: fmtPct(churnRate), helper: churnedPeriod > 0 ? `${fmtInt(churnedPeriod)} no período` : 'sem churn', delta: churnedPeriod > 0 ? churnDelta.delta : '', deltaTone: churnDelta.deltaTone, icon: <ChartColumnIcon size={14} />, progress: progressFromChurn(churnRate), progressTone: 'risk', tone: toneFromChurn(churnRate) },
    ];
  }, [activeClients, churnRate, churnedPeriod, currentMonthNewClients, mrr, period, previousMetrics, revenueLost, revenueNew, ticketMedio, totalClients]);
  ...
  useEffect(() => {
    const title = (
      <>
        ...
        <span className={styles.headerTitlePeriod}>{periodLabel}</span>
      </>
    );
    ...
    const actions = (
      <div className={styles.toolbar}>
        <Select type="squad" value={squadFilter} onChange={(event) => setSquadFilter(event.target.value)} aria-label="Filtrar por squad" disabled={isSquadsLoading} placeholder={isSquadsLoading ? 'Carregando squads…' : 'Todos squads'}>
          <option value="">Todos squads</option>
          {squadsList.map((squad) => (
            <option key={squad.id} value={squad.id} data-avatar={getSquadAvatar(squad) || squad.avatarUrl || squad.logoUrl || ''} data-name={squad.name}>{squad.name}</option>
          ))}
        </Select>
        ...
        <Select value={`${period.y}-${period.m}`} onChange={handlePeriodChange} aria-label="Selecionar período">
          {periodOptions.map((option) => <option key={`${option.y}-${option.m}`} value={`${option.y}-${option.m}`}>{option.label}</option>)}
        </Select>
      </div>
    );

    setPanelHeader({ title, description: '', actions });
  }, [clientFilter, clientOptions, period, periodLabel, periodOptions, setPanelHeader, shellLoading, squadFilter, squads]);
    ...
    return (
      <div className={styles.page}>
        <div className={styles.stateBlock}>
          ...
          <button type="button" className={styles.retryButton} onClick={refreshClients}>Tentar novamente</button>
        </div>
      </div>
    );
  }
